...
|---|---|---|
| **MAST-Data** — [`huggingface.co/datasets/mcemri/MAD`](https://huggingface.co/datasets/mcemri/MAD), code at [`github.com/multi-agent-systems-failure-taxonomy/MAST`](https://github.com/multi-agent-systems-failure-taxonomy/MAST) | `MAD_full_dataset.json`: 1,642 annotated traces, 7 frameworks (AG2, MetaGPT, ChatDev, Magentic, AppWorld, HyperAgent, OpenManus), 8 benchmarks, 5 LLMs, with a `trajectory` field per trace. `MAD_human_labelled_dataset.json`: 19 traces | **Best candidate.** Cross-framework, already parsed, already annotated. Two open questions to check first: whether trajectories preserve raw message text or only step logs, and whether finish reasons / token counts survive. If they do, the audit is: measure final-turn length distribution against the framework's configured cap, per arm |
