...
|---|---|---|---|
| 1.1 | Second task family | ✅ **measured** | `tasks/code/`, 1,050 episodes, preregistered in `docs/PREREG-code.md` before scoring. **C1 falsified on its truncation clause**: the verbosity ratio transfers (1.78x) but truncation is 0.0% in both arms — a 3,072-token budget does not bite on this task. **Yet the arm still shows +0.0783 for the team, and all of it is the fourth artifact**: excluding unparseable answers the gap is **-0.0004**. The class transfers; the member does not. Underpowered for the 0.05 margin (realised sd 0.29-0.37 against an assumed 0.15), so a bound of 0.0585 is reported rather than a null — as §4 of the prereg registered in advance |
| 1.2 | Second model family | ✅ **measured (qwen); mistral withdrawn** | **Qwen2.5-7B, 450 episodes: gap -0.0059, equivalence bound 0.0288 — the tightest in the study. F1 confirmed, the null is not a Llama quirk.** And the surprise: **qwen's malformed rate is 0.0% in both arms** against Llama's 16.8% solo / 0.0% team, so the fourth artifact is a model-by-parser interaction rather than a property of answer parsing. **Mistral withdrawn, not by choice**: its chat template forbids a `system` role and requires alternating roles, so this study's briefs are inexpressible in it; running it would have confounded family with prompt assembly. Amendment in `docs/PREREG-families.md`. Yield: **the multi-agent brief is not portable across chat templates** |
| 1.3 | Run the 14B | ❌ *(built + preregistered; **cut by decision** 2026-08-22)* | Qwen2.5-14B Q4_K_M on disk, 3-slot geometry derived, `docs/PREREG-14b.md` registered with a three-clause threshold, MDE 0.049 at n=150, and a declared scale/family confound. **Cut rather than failed**: 15 card-hours for an answer that arrives confounded, while five corpora sat unscored and the deadline was seven days out. The Ringelmann paper (`RELATED.md`) further reduces its value — team-size scaling is now their contribution. Uncommenting one call in `queue-tier1.sh` restores it; nothing regenerates |
| 1.4 | Precision/quantization control | ⚠️ **one rung measured; the registered test is untested** | Q8_0 ran: 450 episodes, gap +0.0535 against Q4's +0.0813, bound 0.089. **f16 was cut on 2026-08-22** when analysis rather than generation became the constraint. The registered quantity in `PREREG-families.md` is the *interaction* — whether the team-minus-solo gap grows with precision — and **one rung cannot produce a difference of differences**. Q4→Q8 moves the gap by −0.028, the opposite direction from PAPER-DRAFT §9's worry, and is recorded as **untested rather than as reassurance**. Configs, weights and the ladder are intact and resume from disk |
...
| 1.8 | Effect-size CIs in main tables | ✅ | Bootstrap intervals were already computed; now `build_paper.table()` **refuses at build time** to render a p-value column with no effect size or interval beside it. The lint fired on the paper's own central table, which has gained the equivalence-bound column it should have had |
...
| 1.10 | Academic prose, not log register | ✅ | Register split now **declared** (`docs/SUPPLEMENTARY.md`) and **enforced** (`check_register` refuses log references, first person singular, dates and diary markers in the paper body). Audited the existing prose: no leakage — the `§4.x` hits are the paper citing itself |
...
| 1.15 | README reproduces the headline end-to-end | ✅ *(one corpus; the second was found missing 2026-08-21)* | **Correction.** The headline gate reproduces. The *paper* did not build: `build_paper.py` also reads `runs/llama31-8b-q4-medium-ans`, the 48-episode pilot at seeds 0–47, for the +0.055 participation effect, the fungibility bound and the whole pilot column. That corpus was never released — no `corpus-v1` release exists, verified 404 — and `runs/` is gitignored, so the rebuild restored one of the two corpora the paper is assembled from. Regenerating it, **as a re-measurement rather than the original**: sampling is at temperature 0.8 and continuous batching is not bit-reproducible, so the restored numbers are compared against the recorded ones rather than assumed equal. This is §1.0.a still partly live. |

### Tier 2 — materially raises the odds

| # | Item | Status | Where it stands |
|---|---|---|---|
| 2.1 | Audit a public multi-agent system for the same artifact | ❌ *(targets surveyed; no audit run)* | `docs/AUDIT-TARGETS.md`. Best target is **AlpacaEval's `results/`** — Together-MoA and Self-MoA against their own single-model baselines, 805 records per arm, ~7 MB over plain HTTPS, the only candidate found with two arms on a matched task set. **Two findings already**: RELATED.md §6's top candidate (MAST-Data) has no solo arm and cannot answer this at all; and artifact 4 is confirmable from source without any corpus, because MoA, Du et al. and ChatDev all discard the stop reason at the call site |
| 2.2 | Release the diagnostic as a usable tool | ✅ | `collabengine audit <transcripts>` — foreign-schema adapter over `analysis/integrity.py`, schema in `docs/AUDIT-SCHEMA.md`. **Degrades rather than guesses**: an absent `finish_reason` makes truncation unavailable instead of inferred. A clean audit exits 0 and says so, because making a finding an error gives anyone auditing their own system a reason not to run it |
| 2.3 | Replace or demote the κ = 0.29 judge | ❌ *(measured; the decision is the author's)* | **The free check ran and κ did not move** (§4.25). Best 8B 0.288, best 14B 0.262 — 75% more parameters buys nothing, so §4.17's capacity prediction is **not supported** and the cheapest excuse for keeping Phase 2 in the body is gone. `v3 coarse` excluded: it emits `check/meta/solve` against human `propose/verify/…`, so its κ = 0.000 is a label-space mismatch, not a judge failure. **Recommended resolution is now demotion to the appendix** — the ablation-side nulls do not depend on Phase 2. `docs/ADVISOR-BRIEF.md` §5(c) |
| 2.4 | Dose-response cap sweep | ✅ **measured — the strongest new result** | 1,000 episodes across caps 512/1024/2048/3072/6144. Apparent team advantage **+0.390 → +0.100 → +0.069 → +0.026 → +0.043**. Registered contrast wanted ≥ +0.10 and measured **+0.347**. **At a tight cap the instrument manufactures a larger team advantage than any real gain in the frameworks surveyed in `RELATED.md`.** Makes the diagnostic predictive rather than descriptive. Caveat recorded: the verbosity ratio is *not* monotone in the cap even though the advantage is |
...
| 3.2 | Anonymize per double-blind policy | ✅ | `--anonymous` exercised for the first time. Strips byline, affiliation, contact, acknowledgements, **the docx author property** (which survives export and shows in any file inspector), the output filename (named after the repo), and the code link — replaced by a release-upon-acceptance statement rather than deleted. `tests/test_anonymize.py` greps the **rendered** document, not the code that writes it |
