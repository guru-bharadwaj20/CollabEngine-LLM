...
| 2.3 | Replace or demote the κ = 0.29 judge | ❌ *(free check queued)* | Four codebooks moved κ from 0.07 to 0.29 and stopped (§4.17). `scripts/ops/queue-judge-14b.sh` re-runs the 40-message hand-coded set through the 14B on disk — minutes of card — which settles capacity-limit versus wording. **Queued, not yet run.** The decision it feeds is in `docs/ADVISOR-BRIEF.md` §5(c) |
