...
| 1.2 | Second model family | ✅ *(qwen generated; mistral withdrawn on an instrument incompatibility)* | **Qwen2.5-7B: 450 episodes, complete, unscored.** **Mistral-7B-v0.3 is dropped and not by choice**: its chat template raises `Conversation roles must alternate` and has no `system` role, while `episode.py` sends both. Running it would have required a different prompt assembly for one family — confounding family with assembly, §4.1c's mistake relabelled. Registered as an amendment in `docs/PREREG-families.md`. **The real yield is a limitation worth stating: the study's multi-agent brief is not portable across chat templates.** Preflight caught it for one probe request instead of a card-night |
...
| 2.4 | Dose-response cap sweep | ❌ *(built + preregistered; queued)* | `cap-sweep.sh` across 512/1024/2048/3072/6144, `docs/PREREG-cap-sweep.md` registering the predicted curve shape and per-rung MDE before the run, `dose_response.py` for the curve. **Queued behind the pilot; 3–4 card-nights** |
